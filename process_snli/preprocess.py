...
def get_data(args):
    ...
    def make_vocab(srcfile, targetfile, labelfile, seqlength):
        ...
        for _, (src_orig, targ_orig, label_orig) in \
            ...
            src_orig = word_indexer.clean(src_orig.strip().lower())
            targ_orig = word_indexer.clean(targ_orig.strip().lower())                
            targ = targ_orig.strip().lower().split()
            src = src_orig.strip().lower().split()
            label = label_orig.strip().lower().split()
    ...
    def convert(srcfile, targetfile, labelfile, batchsize, seqlength, outfile, num_sents,
        ...
        for _, (src_orig, targ_orig, label_orig) in \
            ...
            src_orig = word_indexer.clean(src_orig.strip().lower())
            targ_orig = word_indexer.clean(targ_orig.strip().lower())
            targ =  [word_indexer.BOS] + targ_orig.strip().lower().split()
            src =  [word_indexer.BOS] + src_orig.strip().lower().split()
            label = label_orig.strip().lower().split()
