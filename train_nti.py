...
from preprocessing import preprocess4, batch, load_data
...
hypothesis, premise, labels, lbls_enc = load_data(input_dir + '/snli_1.0_train.jsonl')
snli_train = pd.DataFrame()
snli_train['sentence1'] = pd.Series(premise)
snli_train['sentence2'] = pd.Series(hypothesis)
snli_train['gold_label'] = pd.Series(labels)

hypothesis, premise, labels, lbls_enc = load_data(input_dir + '/snli_1.0_dev.jsonl')
snli_dev = pd.DataFrame()
snli_dev['sentence1'] = pd.Series(premise)
snli_dev['sentence2'] = pd.Series(hypothesis)
snli_dev['gold_label'] = pd.Series(labels)

hypothesis, premise, labels, lbls_enc = load_data(input_dir + '/snli_1.0_test.jsonl')
snli_test = pd.DataFrame()
snli_test['sentence1'] = pd.Series(premise)
snli_test['sentence2'] = pd.Series(hypothesis)
snli_test['gold_label'] = pd.Series(labels)

del hypothesis, premise, labels, lbls_enc
...
for i in xrange(0, n_epoch):
	...
	#six.moves.range is a python 2/3 compatibility function
	...
	logging.info("dev accuracy_score: %s" %(f1_dev))
	# print confusion_matrix(preds_true, preds)
	if f1_dev > max_dev:
		preds = []
